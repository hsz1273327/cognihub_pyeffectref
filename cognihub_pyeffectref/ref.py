...
class Ref(Generic[T]):
    ...
    def subscribe(self, callback_func: 'EffectWrapper' | Callable[[T, T], Any]) -> None:
    ...
    def _notify_subscribers(self, old_value: T, new_value: T) -> None:
                ...
                if isinstance(callback, EffectWrapper):
                    ...
                    if callback._is_async:
                        ...
                        try:
                            ...
                            # # 获取协程对象并创建任务
                            # coro = callback.run_triggered_effect(new_value, old_value)
                            # if asyncio.iscoroutine(coro):
                            #     loop.create_task(coro)
                            asyncio.create_task(callback.run_triggered_effect(new_value, old_value))
                        except RuntimeError:
                        ...
                        if self._global_sync_executor_config:
                            if self._global_sync_executor_config == 'asyncio':
                                ...
                                asyncio.create_task(asyncio.to_thread(callback.run_triggered_effect,new_value, old_value))
                            else:
                    ...
                    if asyncio.iscoroutinefunction(callback):
                        ...
                        try:
                            # loop = asyncio.get_running_loop()
                            # loop.create_task(func(new_value, old_value))
                        ...
                        if self._global_sync_executor_config:
                            if self._subscribe_immediate:
                                ...
                                if self._subscribe_sequential:
                                    ...
                                    if self._global_sync_executor_config== 'asyncio':
                                        # 如果全局配置为 asyncio，直接调度到事件循环
                                        asyncio.create_task(asyncio.to_thread(callback,new_value, old_value))
                                    else:
                                        # 如果全局配置为自定义 Executor，使用它来执行同步任务
        ...
        if len(subscribers_to_notify_sequential_bg) > 0:
            ...
            if self._global_sync_executor_config:
                if self._global_sync_executor_config == 'asyncio':
                    # 如果全局配置为 asyncio，直接调度到事件循环
                    asyncio.create_task(asyncio.to_thread(self._run_sequential_bg, subscribers_to_notify_sequential_bg, old_value, new_value))
                else:
                    # 如果全局配置为自定义 Executor，使用它来执行同步任务
